...
import { DONATION_URL } from '../shared/constants';
    ...
    <div className="flex h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
      ...
      <aside className="w-64 bg-[var(--bg-secondary)] border-r border-[var(--border)] flex flex-col">
          ...
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className={`block w-full text-left px-4 py-2 rounded-lg transition-colors ${
                activePage === item ? 'bg-apex-cyan/10 text-apex-cyan' : 'hover:bg-[var(--hover)] text-[var(--text-secondary)]'
              }`}
            >
              {item}
            </button>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="text-xs text-[var(--text-muted)]">ApexPulse v1.0.0</div>
        </div>
      </aside>
